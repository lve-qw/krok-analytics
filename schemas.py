...
class DialogMetadata(BaseModel):
    ...
    periodicity: str = Field(default="none", description="Периодичность")
    complexity: str = Field(default="simple", description="Сложность")
    steps_requested: int = Field(default=1, description="Количество запрошенных шагов")
    ...
    uses_company_data: bool = Field(default=False, description="Использует ли внутренние данные")
    company_sources: List[str] = Field(default_factory=list, description="Источники внутренних данных")
    requires_generation: List[str] = Field(default_factory=list, description="Типы генерации")
    search_type: List[str] = Field(default_factory=list, description="Типы поиска")
    contains_sensitive_data: bool = Field(default=False, description="Есть ли чувствительные данные")
    prompt_injection: bool = Field(default=False, description="Попытка промпт-инъекции")
    agent_failed: bool = Field(default=False, description="Неудача агента")
