...
## 11. Статус реализации (обновление после решений команды)

Решения приняты, шаги 1–4 выполнены. Итог:

| Шаг | Статус | Что сделано |
|---|---|---|
| 1. Перенос контракта | Готово | `analytics_contract/`, `data/classes_31.csv`, `docs/metrics_catalog.md`, тесты |
| 2. Адаптер | Готово | `analytics_export.py` |
| 3. Валидация | Готово | 113/115 строк, 0 ошибок контракта |
| 4. Dashboard | Готово | `analytics_contract/dashboard/`, 7 KPI, 7 диаграмм, drill-down, экспорт |
| 5. Правки качества | Готово | см. ниже |

### Как разрешены блокеры §9

1. **Таксономия.** Сопоставление 23 `scenario_id` → 31 класс из §4.2
   **не применяется**. Канонической таксономией стали 31 класс из
   `data/classes_31.csv`; `utils.load_classes` читает их, `ZeroShotClassifier`
   строится на них. `scenario_id` и 10 широких категорий остаются legacy/demo
   метаданными. Таблица §4.2 сохранена как справочный материал для отладки, а
   не как механизм.
2. **Строки `parse_error`.** Исключаются в `outputs/pipeline_errors.csv`,
   `agent_failed` от этого не выставляется. Счётчики входных, обработанных и
   ошибочных записей — в `outputs/export_report.json`.
3. **Инверсия `confidence` (§5.3).** Исправлена: при непрохождении порога
   берётся argmax-класс со **своим фактическим** скором вместо синтетической
   `1.0`.

### Прочие изменения

- `FIXED_TOOLS` / `FIXED_INTEGRATIONS` подключены к нормализации (`llm.py`,
  `analytics_export.py`) и к валидации (код `registry_unknown`). Неопознанные
  имена отбрасываются и перечисляются в `export_report.json`.
- `requires_generation` фильтруется по закрытому словарю, как `search_type`.
- `tool_tokens = 0` зафиксирован как ограничение инструментирования в отчёте
  валидации, в каталоге метрик и в подписи диаграммы 6.
- `estimated_cost` описан как стоимость LLM/API-обработки; инфраструктура и ROI
  вынесены в ограничения.
- Legacy-выход (`dialogs.csv`, `use_cases.csv`, `analytics.csv`) сохранён.

### Что осталось непроверенным

Настоящий pipeline не запускался: нужен GPU и загрузка Qwen2.5-7B. Цепочка
проверена на синтетическом выходе `scripts/make_sample_pipeline_output.py`,
повторяющем формат `utils.save_dialogs_csv`. Первый прогон на реальном выходе
может выявить классы ошибок, не предусмотренные адаптером — для этого и
существует `pipeline_errors.csv`.

Дашборд не был осмотрен визуально: в системе нет браузера для рендеринга.
Проверено структурно — сервер отвечает 200, все фигуры собираются в обеих
темах, ни одна не содержит второй оси Y.
