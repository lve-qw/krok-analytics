...
|---|---|---|---|---|
| 1 | Проанализировано диалогов | `vol.total_dialogues`, `vol.work_share` | Каков масштаб и по делу ли используют агента? | Расширять или сужать охват платформы |
| 2 | Зафиксировано сбоев | `fail.rate` | Насколько агент надёжен? | Ставить задачи владельцам проблемных сценариев |
| 3 | Кандидаты на автоматизацию | `auto.candidate_share` | Какая часть нагрузки уходит в автоматику? | Формировать очередь автоматизации |
| 4 | Стоимость обработки | `cost.total`, `cost.per_dialogue` (медиана) | Сколько стоит эксплуатация? | Планировать бюджет |
| 5 | Концентрация расходов, топ-5 | `cost.concentration` | Даст ли работа с немногими сценариями основную экономию? | Оптимизировать контекст в топ-сценариях |
| 6 | Требуют ручного разбора | `drv.review_queue` | Сколько записей нельзя принимать на веру? | Назначить разбор аналитику |
| 7 | Индикаторы риска данных | `sec.sensitive_rate`, `sec.injection_rate`, `sec.exposure_overlap` | Есть ли угроза утечки и атак? | Включать контроль доступа и фильтрацию контекста |

Проверка по требованиям §5.2 задачи: каждая карточка отвечает на отдельный
вопрос, считается из валидированных полей, ведёт к действию, не дублирует
соседнюю и названа без технического жаргона.

Карточка 4 показывает **медиану**, а не среднее: распределение стоимости
тяжелохвостое, среднее смещается несколькими дорогими диалогами.

## 4.2. Основные диаграммы (7)

| # | Диаграмма | Тип | Метрики | Отвечает на вопрос |
|---|---|---|---|---|
| 1 | Топ сценариев по объёму | горизонтальный bar, топ-15 | `vol.by_use_case`, `vol.by_class` (переключатель) | Что используют чаще всего? |
...
второе — переключателем в диаграмме 1. Это удерживает первый экран в границах
5–7 диаграмм, заданных задачей.
